 ...
 */
//=============================================================================
#ifndef INTELLIVUE_MP70_SRC_INTELLIVUE_H_
#define INTELLIVUE_MP70_SRC_INTELLIVUE_H_

#include <stdio.h>
#include <iostream>
#include <stdint.h>
//#include "intellivueMP70Members.h"
...
namespace intellivue
{
  ...
  // Fails most of the time with pragma pack(1)
 ...
 *
 * The ProtocolVersion bit field contains the supported versions of the Data Export protocol. The
 ...
 *
 * The NomenclatureVersion bit field contains the revision of the nomenclature which is used to
 ...
 *
 * The FunctionalUnits activates additional protocol functions. The Computer Client must set
 ...
 *
 * The SystemType bit field indicates whether the device is a Computer Client or a server. The
 ...
 * refused.
 *
 * The StartupMode indicates the startup mode of the Computer Client and the IntelliVue
 ...
 * supported, the Association Request is refused. The IntelliVue monitor supports the following profiles:
 *
 ...
 * SINGLE POLL DATA REQUEST on page 45.
 *
 ...
 * -- The IntelliVue monitor sets max_mtu_tx to 600 bytes (the IntelliVue monitor can not receive larger
 ...
 * the number of internal objects (e.g. the number of Numerics) may change dynamically.
 *
 ...
 * averaged data as source for Numeric data.
 *
 * The Computer Client must set at least one of the bits, otherwise the optional package is ignored.
 * Currently, the IntelliVue monitor supports only one source for an association. If more than one of the
 * bits is set, the source with the smallest measurement period is selected. The IntelliVue monitor sets the
...
/**
 * poll_number
 * This field will be sent back in the response message. It is recommended to use
 * this field as a counter.
 *
 * polled_obj_type
 * Defines which objects (Numerics or Alarms or MDS or Patient Demographics) is polled.
 * The following lists supported objects and their corresponding TYPE values:
 *
...
/// naming context (IntelliVue monitor supports multiple naming contexts).
typedef struct {
  MdsContext context_id;
  struct {
    uint16_t count;
...
/// The PollMdibDataReply structure is the top level data structure returned in the Single Poll Data
/// Result message.
typedef struct {
  uint16_t poll_number;
...
/**
 * The Display Resolution attribute is present if the resolution of the numeric shown on the display must
 * be different from the resolution communicated in the Numeric Observed Value attribute. E.g. a
 * Temperature is displayed with a resolution of 1/10, but the Observed Value is sent with a precision of
 * 1/100 to get the necessary accuracy for differential temperatures. The Display Resolution attribute
 * describes the format in which the value of a numeric is displayed on the screen
 */
typedef struct {
  uint8_t pre_point;
  uint8_t post_point;
...
/**
 * AVAIL_INTERMITTEND: The intermitted availability bit is set, if the observed values not always
 * available (e.g. only if a measurement is explicitly started).
 * UPD_PERIODIC: observed value is updated periodically
 * UPD_EPISODIC: observed value is updated episodically (exactly one update mode (UPD_) must
 * be set
 * MSMT_NONCONTINUOUS: indicates that the measurement is non continuous (this is
 * different from the update mode)
 */


/**
 * The MetricStructure describes if the object represents a single measurement or multiple
 * related measurements (an invasive blood pressure could be compound when it represents
...
enum IntellivueMeasurementState
{
  intellivue_invalid = 0x8000,
  intellivue_questionable = 0x4000,
  intellivue_unavailable = 0x2000,
  intellivue_calibration_ongoing = 0x1000,
  intellivue_test_data = 0x0800,
  intellivue_demo_data = 0x0400,
  intellivue_validated_data = 0x0080,
  intellivue_early_indication = 0x0040,
  intellivue_msmt_ongoing = 0x0020,
  intellivue_msmt_state_in_alarm = 0x0002,
  intellivue_msmt_state_al_inhibited = 0x0001
};
 ...
 * QUESTIONABLE: A problem exists, but it is still appropriate to present the data. This occurs
 ...
 * UNAVAILABLE: The signal does not permit derivation of the numeric in question. This could be a
 ...
 * TEST_DATA: The signal is an automatically generated test signal only and is not a valid patient
 * signal. If this bit is set, the value is not suitable for patient diagnosis.
 * DEMO_DATA: The IntelliVue monitor runs in demonstration mode, the signal is automatically
 ...
 * EARLY_INDICATION: The value represents an early estimate of the actual signal (the Non-
...
/// The Numeric Observed Value attribute represents the (measured) value, along with state and
/// identification data.
typedef struct {
  OIDType physio_id;
...
/**
 * The Compound Numeric Observed Value attribute represents multiple (measured) values modelled in
 * one Numeric object, along with state and identification data.
 * The Compound Numeric Observed Value is e.g. used to represent Blood Pressure measurements. For
 * these measurements, systolic, diastolic and mean values are represented by a single Numeric object.
 */
typedef struct {
  uint16_t count;
  uint16_t length;
...
/// The Sample Array Calibration Specification allows to define the presence of a calibration bar or
/// calibration stair.
typedef struct {
 ...
 *
 * The Sample Array Observed Value attribute represents the wave samples, along with state and
 * identification data.
 */
typedef struct {
  uint16_t count;
 ...
 * The number of significant_bits is less or equal sample_size. To get the actual sample value, nonsignificant
 * bits must be masked if indicated in the flags value.
 */
...
/// The Scale and Range Specification describes a relation between scaled values and absolute values and
/// also defines the range of the measured values and samples.
typedef struct {
  FLOATType lower_absolute_value;
  FLOATType upper_absolute_value;
  uint16_t lower_scaled_value;
 ...
 * UNASSOCIATED: The IntelliVue monitor is connected to the network, but no association is
 * currently active.
 * OPERATING: The IntelliVue monitor has an association with a Computer Client.
 * Currently, a Computer Client will only see the MDS Status OPERATING, if the MDS has another
 * Status, there is no association with a Computer Client.
 */
...
/// The Sample Array Fixed Value Specification defines a list of fixed sample values or bit masks that
...
#define MDS_GEN_SYSTEM_INFO_SYSTEM_PULSE_CHOSEN 1
  uint16_t length;
  uint8_t value[1]; /// placeholder for appended data
...
/// The Mds General System Info attribute contains global information about the monitor and its
/// configuration
typedef struct {
  uint16_t count;
  uint16_t length;
