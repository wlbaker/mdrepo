/**
 * @file  NumericContextReaderInterface.cxx
...
#include "NumericContextReaderInterface.h"

// -------------------------------------------------------------------------
NumericContextReaderInterface::NumericContextReaderInterface()
  : _communicator(0),
    _reader(0),
	  _waitSet(0),
	  _condition(0),
...
int NumericContextReaderInterface::Destroy()
{
...
int NumericContextReaderInterface::Initialize(
  const long domain_id,
  const char *topicname,
  const char *qoslibraryname,
  const char *qosprofilename)
{
  const char *sfn {"NumericContextReaderInterface::Initialize"};
  DDS_ReturnCode_t rc {DDS_RETCODE_OK};
  ...
  // Create topic
  DDS::Topic *topic =
    _communicator->CreateTopic<ice::NumericContext>(topicname);
  ...
  _reader = ice::NumericContextDataReader::narrow(reader);
  if (!_reader)
  {
    std::stringstream errss;
...
// -------------------------------------------------------------------------
// Wait for data
void NumericContextReaderInterface::WaitForData()
{
  const char *sfn {"NumericContextReaderInterface::WaitForData"};
  DDS::ConditionSeq active_conditions;
  DDS::SampleInfoSeq info_seq;
  ice::NumericContextSeq data_seq;
    ...
    for (int i = 0; i < active_conditions.length(); ++i)
    {
      if (active_conditions[i] == _instance_alive)
      {
        ...
        for (int j = 0; j < data_seq.length(); j++)
        {
          ...
          ice::NumericContextTypeSupport::print_data(&data_seq[j]);
        }
      ...
      else if (active_conditions[i] == _instance_disposed)
      {
        ...
        for (int j = 0; j < data_seq.length(); j++)
        {
          ...
          ice::NumericContext *data_instance =
...
// -------------------------------------------------------------------------
// Get data
bool NumericContextReaderInterface::GetData(
  const DDS_Duration_t &timeout,
  ice::NumericContext *data_sample)
{
  const char *sfn {"NumericContextReaderInterface::GetData"};
  ...
  for (int i = 0; (i < active_conditions.length()) && !have_data; ++i)
  {
    if (active_conditions[i] == _instance_alive)
    {
      ...
      for (int j = 0; j < data_seq.length(); j++)
      {
        ...
        ice::NumericContextTypeSupport::copy_data(data_sample, &data_seq[j]);
        have_data = true;
        break;
      }
      _reader->return_loan(data_seq, info_seq);
