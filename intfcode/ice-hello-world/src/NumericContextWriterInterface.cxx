 /**
 * @file  NumericContextWriterInterface.cxx
 * @brief Manage DDS entities for writing the NumericContextValue type.
 */
//=============================================================================
...
#include "NumericContextWriterInterface.h"

// ----------------------------------------------------------------------------
NumericContextWriterInterface::NumericContextWriterInterface()
  : _communicator(0),
    _writer(0)
{
}
...
int NumericContextWriterInterface::Destroy()
{
...
int NumericContextWriterInterface::Initialize(
  const long domain_id,
  const char *topicname,
  const char *qoslibraryname,
  const char *qosprofilename)
{
  const char *sfn {"NumericContextWriterInterface::Initialize"};
  DDS_ReturnCode_t rc {DDS_RETCODE_OK};
  ...
  // Downcast the generic dataw riter to a DataWriter of specific type
  _writer = ice::NumericContextDataWriter::narrow(writer);
  if (!_writer)
  {
    std::stringstream errss;
...
int NumericContextWriterInterface::Write(DdsAutoType<ice::NumericContext> data)
{
...
// notify other applications that this data instance went away and should be deleted.
int NumericContextWriterInterface::Delete(DdsAutoType<ice::NumericContext> data)
{
  DDS_ReturnCode_t rc {DDS_RETCODE_ERROR};  // assume fail
  if (_writer)
  {
