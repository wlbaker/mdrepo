 /**
 * @file  NumericWriterInterface.cxx
...
// ----------------------------------------------------------------------------
NumericWriterInterface::NumericWriterInterface()
  : _communicator(0),
    _writer(0)
{
}
...
int NumericWriterInterface::Destroy()
{
...
int NumericWriterInterface::Initialize(
  const long domain_id,
  const char *topicname,
  const char *qoslibraryname,
  const char *qosprofilename)
{
  const char *sfn = "NumericWriterInterface::Initialize";
  DDS_ReturnCode_t rc {DDS_RETCODE_OK};
  ...
  _writer = ice::NumericObservedValueDataWriter::narrow(writer);
  if (!_writer)
  {
    std::stringstream errss;
...
int NumericWriterInterface::Write(DdsAutoType<ice::NumericObservedValue> data)
{
...
int NumericWriterInterface::Delete(DdsAutoType<ice::NumericObservedValue> data)
{
  DDS_ReturnCode_t rc {DDS_RETCODE_ERROR};  // assume fail
  if (_writer)
  {
