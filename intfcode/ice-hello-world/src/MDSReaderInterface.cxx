...
// -------------------------------------------------------------------------
MDSReaderInterface::MDSReaderInterface()
  : _communicator(0),
    _reader(0),
	  _waitSet(0),
	  _condition(0),
...
int MDSReaderInterface::Destroy()
{
...
int MDSReaderInterface::Initialize(
  const long domain_id,
  const char *topicname,
  const char *qoslibraryname,
  const char *qosprofilename)
{
  const char *sfn {"MDSReaderInterface::Initialize"};
  DDS_ReturnCode_t rc {DDS_RETCODE_OK};
  ...
  _reader = ice::MDSDataReader::narrow(reader);
  if (!_reader)
  {
    std::stringstream errss;
...
// -------------------------------------------------------------------------
// Wait for data
void MDSReaderInterface::WaitForData()
{
  const char *sfn {"MDSReaderInterface::WaitForData"};
    ...
    for (int i = 0; i < active_conditions.length(); ++i)
    {
      if (active_conditions[i] == _instance_alive)
      {
        ...
        for (int j = 0; j < data_seq.length(); j++)
        {
          ...
          ice::MDSTypeSupport::print_data(&data_seq[j]);
        }
...
// -------------------------------------------------------------------------
// Get data
bool MDSReaderInterface::GetData(
  const DDS_Duration_t &timeout,
  ice::MDS *data_sample)
{
  const char *sfn {"MDSReaderInterface::GetData"};
  ...
  for (int i = 0; (i < active_conditions.length()) && !have_data; ++i)
  {
    if (active_conditions[i] == _instance_alive)
    {
      ...
      for (int j = 0; j < data_seq.length(); j++)
      {
        ...
        ice::MDSTypeSupport::copy_data(data_sample, &data_seq[j]);
        have_data = true;
        break;
      }
      _reader->return_loan(data_seq, info_seq);
