...
public class XWilkinson {
	...
	public Label search(double dmin, double dmax, int m) {
		Label label;
		if (dmax == dmin || m == 0 || Double.isNaN(dmin) || Double.isNaN(dmax)) {
			return null;
		}
		...
		if (dmax < dmin) {
			// inverted
			label = _search(dmax, dmin, m);
		} else {
			// standard
			label = _search(dmin, dmax, m);
		}

		double tol = label.step / 100;
		if (label.min + tol < dmin) {
			label.min += label.step;
		}
		if (label.max - tol > dmax) {
			label.max -= label.step;
		}

		return label;
	}

	public Label _search(double dmin, double dmax, int m) {
