...
update <- function( stopSecs ) {
	...
	s3d <- scatterplot3d( x=df$pressure, y=df$dpdt, z=df$time,  
		angle=angle, pch=16, color='white', zlim=c(0,0.5), xlim=c(30,110), ylim=c(-500,1000), 
		col.axis="blue", col.grid="lightblue", 
		zlab="Time", xlab="Pressure", ylab="dP/dt", 
	...
	for( ii in c(2:nbeats) ) {
		...
		s3d$points3d( x=df$pressure, y=df$dpdt, z=df$time, highlight.3d=TRUE, col=alpha('black',0.2), pch=16, fill=TRUE )
	}
