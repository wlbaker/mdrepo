...
update <- function( stopSecs ) {
	...
	dat <- melt(output)
	names(dat) <- c("x", "y", "z")
	dat$y = dat$y * 1000/freq # convert to ms

	v <- ggplot( dat,aes(x=x,y=y,z=z) ) + theme_bw() +
		geom_raster(aes(fill=z)) + geom_contour(color="white")
	v <- v + scale_y_continuous("Time (ms)") + scale_x_continuous("Beat")

	v <- v + theme(plot.margin = unit( c(0.1,0.1,0.1,0.1), "cm" ) )
