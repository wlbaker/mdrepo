...
yy <- times[(y_lag+1):n] - times[1:(n-y_lag)]
n <- length(yy)
...
# optomized for RRI
p <- p + xlim(0.4,1.4) + ylim(0.4,1.4)
p <- p + xlab( title ) + ylab(sub_title)
p <- p + geom_jitter(aes(x = xx, y=yy), position = position_jitter(width = .0005, height=0.0005), alpha = 0.2) 
print(p)
