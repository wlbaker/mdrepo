...
public class ideaPrintServiceExporter extends JRAbstractExporter implements Printable
{
			...
			for(reportIndex = 0; reportIndex < jasperPrintList.size(); reportIndex++)
			{
				...
				try 
				{
					...
					if (displayPageDialog)
					{
						...
						PageFormat pf = printerJob.pageDialog(printRequestAttributeSet);
						if( pf == null ) {  // wlb...cancel on Cancel
							return;
						}
					}
					...
					else
					{
						...
						// WLB: Use this method, as MediaPrintableArea
						// is respected, but PageFormat areas are not.
						
						// also, must respect the printRequest orientation,
						// not just the Jasper orientation
						
						// TODO: fixme the Jasper orientation is ignored here!
						
						Attribute orientation = printRequestAttributeSet.get( OrientationRequested.class);

						int h_debug = jasperPrint.getPageHeight();
						int w_debug = jasperPrint.getPageWidth();
						
						// TODO: why don't h_debug and w_debug match the template values
						
						float height = jasperPrint.getPageHeight() / 72.0f;
						float width = jasperPrint.getPageWidth() / 72.0f;
						
						if( orientation != null && orientation == OrientationRequested.LANDSCAPE ) {
							float tmp = width;
							width = height;
							height = tmp;
						}
						...
						// pageFormat.setPaper( paper );   // WLB!!!
						MediaPrintableArea area = new MediaPrintableArea( 0.0f, 0.0f, width, height, MediaPrintableArea.INCH );
