...
public class LoadWizard extends JPanel implements WizardMasterInterface {

	private final static String STUDY_TIME = "Start Time";
	private final static String STUDY_SELECTION = "Selection";
	private static final String STUDY_FILE = "Timepoint Definition";
	...
	StartTimePanel loadTimePanel;
	WizRemoteEntryPanel remoteEntryPanel;
	LoadFileSelection loadSelectionPanel;
	...
	public LoadWizard() {
		...
		safeAdd(loadTimePanel = new StartTimePanel(this), STUDY_TIME);
		safeAdd(loadSelectionPanel = new LoadFileSelection(this), STUDY_SELECTION );
		safeAdd(remoteEntryPanel = new WizRemoteEntryPanel( this, dc = new RpDevice()), STUDY_FILE);

		buildWizardNavigationTree();
		tree.setSelectionInterval(0, 0);

		setCurrentPanel( STUDY_TIME );
	}
	...
	private void buildWizardNavigationTree() {
		...
		DefaultMutableTreeNode time_node = new DefaultMutableTreeNode(STUDY_TIME);
		DefaultMutableTreeNode definition_node = new DefaultMutableTreeNode(STUDY_FILE);

		root.add(time_node);
		root.add( new DefaultMutableTreeNode(STUDY_SELECTION) );
		root.add(definition_node);
	...
	private void nextActionPerformed(ActionEvent e) {
		...
		if (curr.equals(STUDY_TIME)) {
			nextPage = STUDY_SELECTION;
			enableNext( false );
		} else if (curr.equals(STUDY_SELECTION)) {
			nextPage = STUDY_FILE;
			enableFinish( true );
			File file = loadSelectionPanel.getFile();
			remoteEntryPanel.refresh(file );
		} else if (curr.equals(STUDY_FILE)) {
			// finish
		}
	...
	public void finishActionPerformed(ActionEvent e) {
		for (Component c : filler.getComponents()) {
			if (c.isVisible() && c instanceof WizardPanelInterface) {
	...
	public Date getStartTime() {
		return loadTimePanel.getStartDate();
	}

//	public Date getEndTime() {
//		return loadFilePanel.getEndDate();
//	}
	...
	public RpDevice getStudyDefinition() {
		return dc;
	}

	public EntryModel getEntryModel() {
		return remoteEntryPanel.getEntryModel();
	}

	public void setStartTime(Date time) {
		loadTimePanel.setStartTime( time );
	}
