...
@Slf4j
public class NxStageSerialDriver extends SerialAdapter implements AdapterInterface {
	private static final String driverName = "renal/NxStage Serial";
	...
	private static RpMetric[] rateMetric;
	private static RpMetric[] vtbiMetric;
	private static RpMetric[] stateMetric;

	private static RpMetric[] infusedMetric;
	...
	static {
		cap = new DeviceCapabilities(NxStageSerialDriver.class);

		log.error("NOT READY: cap.setMetrics( metrics );");
	}

	public static SerialDeviceConfiguration createConfigInstance() {
		return new SerialDeviceConfiguration(NxStageSerialDriver.class);
	}

	public NxStageSerialDriver(RpDevice conf) {
		super(conf);
	...
	public void sendQueryEvent(int channel, int eventOffset) {
		...
		if (eventOffset < 0) {
			System.err.println("ERROR: Need to wrap around the buffer");
		...
		try {
			...
			while (in.available() > 0) {
				...
				if (localMessage) {
					...
					if (byt == PESC) {
						System.err.println("GOT UNEXPECTED ESCAPE!");
						isEscaped = true;
						continue;
					}
	...
	private void processExtendedMessage() {
		...
		if (ch0 == 0xa8 && mb_buffer[1] == ESC) {
			...
			if (cmd0 == CR || cmd0 == '0') {
				...
				String[] arr = s.split("[ =\"\r\n]+");
				if (arr.length < 3) {
					System.out.println("IGNORING SHORT SHOW COMMAND!");
				} else {
				...
				System.err.println("*******UNKNOWN COMMAND: " + cmd0);
			}
			...
			System.err.println("INVALID CMD FORMAT");
		}
	}
	...
	private void parseEventLogEntry() {
		...
		if (pumpId < 0 || pumpId > 1) {
			System.err.println("[BODYGUARD] INVALID PUMP ID: " + pumpId);
			return;
		}
	...
	private void processLocalMessage() {
		...
		if (pumpId < 0 || pumpId > 1) {
			System.err.println("[BODYGUARD] process local message INVALID PUMP ID: " + s);
			return;
		}
		...
		if (ar.length > 3 && "RUN".equals(cmd)) {
			...
			if (t.equals("RATE")) {
				...
				if (verbose > 0) {
					System.out.println("firing message for pump: " + pumpId);
				}
				fireNumerics(pumpId);
			} else if (t.equals("VTBI")) {
				// ignore...this happens when the INFO key is pressed
			} else {
				System.err.println("[1] UNKNOWN LOCAL MESSAGE: " + s + " T=" + t + " R=" + r);
			}
			...
			if (ar.length == 2) {
				...
				if ("STOP".equals(t)) {
					...
					System.err.println("[Bodyguard] [2] UNKNOWN LOCAL MESSAGE: " + s);
				}
			}
			...
			if (ar.length == 2) {
				...
				if ("RATE".equals(t)) {
					...
					System.err.println("[Bodyguard] [4] UNKNOWN LOCAL MESSAGE: " + s);
				}
			}
		} else {
			System.err.println("[Bodyguard] [3] UNKNOWN LOCAL MESSAGE: " + s);
		}
	}
	...
	public void configureMetrics(RpMetric[] metrics) throws IOException {
		...
		for (RpMetric metric : metrics) {
			...
			if (metricCode.startsWith("0x") && metricCode.length() == 4) {
				...
				System.err.println("NO SUCH METRIC: " + metricCode);
				return; // bad value!
			}
		}
	...
	public Object getMeta(Object key) {
		...
		System.out.println("[BodyGuard] key: " + key + " meta: " + o);
		return o;
	}
	...
	public int tick() {
		...
		if (request_in_progress) {
			...
			if (request_timeout > 2) {
				System.out.println("[NxStage] LAST REQUEST TIMEOUT: " + last_req);
				request_in_progress = false;
			}
		}
