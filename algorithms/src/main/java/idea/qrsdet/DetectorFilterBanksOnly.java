...
public class DetectorFilterBanksOnly extends AbstractDetector {
	...
	public DetectorFilterBanksOnly(int sr) {
		setSampleRate(sr);
		MIN_MSNUM = MS220;
		getConfiguration().setName("DetectorFilterBanksOnly");
	}

	public DetectorFilterBanksOnly(int sr, int minRRI) {
		setSampleRate(sr);
	...
	public int _getValue(int datum) {
		 ...
		 * FNs) but also very often incorrectly detects the presence of a beat
		 * (i.e., noise peaks and has many FPs).
		 * 
		 * This level, thus, serves as an event detector, and is used to
		 * trigger further logic to eliminate FPs introduced here. This level
		 * is designed to have few FNs but it limits the theoretical best FN
		...
		diffSlope = Math.abs(currSlope - prevSlope);
		prevSlope = currSlope;
		 ...
		 * channels use feature F2 in their respective MWIs, however the preset
		 ...
		 * the output in the MWIs of each of and are compared with their
		 ...
		 * rates. Chan1 generates a few FNs but many FPs and Chan2 generates
		 * many FNs but a few FPs.
		 * 
		...
		if ((diffSlope == 2) && ((SL1 != 0) && (NL1 != 0)) && ((SL2 != 0) && (NL2 != 0)) && ((SL4 != 0) && (NL4 != 0))
			 ...
			 * few FPs and, thus, beat detection is very accurate. If both
			 ...
			 * very low, it has very few FNs, and more than likely a beat did
			 ...
			 * generates many FPs but Chan2 generates many FNs. The normalized
			...
			if (((classCh1 == 1) && (classCh2 == 1)) || (classCh2 == 1) || ((classCh1 == 1) && (delta1 > delta2))) {
				 ...
				 * 
				 * This level reduces FNs (events which were inaccurately
				 ...
				 * not-a-beat. Note that since level 4 only checks for FNs, it
		 ...
		 * threshold in its detection logic, it generates few FPs and, thus,
		 ...
		 * Since the threshold used in is very low, it has very few FNs, and
		 ...
		 * many FPs but Chan2 generates many FNs. The normalized detection
	...
	public int getValue(int pending, int datum) {
		...
		if (delay > threshold) {
			fireBeatDetection(-1, 0, -1, leadId, 2);
		}
