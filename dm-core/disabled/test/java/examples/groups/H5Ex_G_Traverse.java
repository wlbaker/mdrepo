...
public class H5Ex_G_Traverse {
    ...
    private static String FILE = "c:/tmp/0000.h5";
...
class H5L_iter_callbackT implements H5L_iterate_cb {
	public H5L_iter_callbackT() {
		System.err.println("* iter callback constructor called");
	}
    public int callback(long object_id, String name, H5L_info_t info, H5L_iterate_t op_data) {
        ...
        try {
            infobuf = H5.H5Oget_info_by_name(object_id, name, HDF5Constants.H5P_DEFAULT);
            System.err.format("%16x ", object_id);
            for (int i = 0; i < spaces; i++)
                System.err.print(" "); // Format output.
            ...
            case H5O_TYPE_GROUP:
                ...
                if (group_check(od, infobuf.addr)) {
                    for (int i = 0; i < spaces; i++)
                        System.err.print(" ");
                    System.err.println("  Warning: Loop detected!");
                }
                else {
                    ...
                    return_val = H5.H5Literate_by_name(object_id, name, HDF5Constants.H5_INDEX_NAME,
                            HDF5Constants.H5_ITER_NATIVE, 0L, iter_cb2, nextod, HDF5Constants.H5P_DEFAULT);
                    // return_val = H5.H5Literate(object_id, HDF5Constants.H5_INDEX_NAME, HDF5Constants.H5_ITER_NATIVE, 0L, this, od);
                }
                for (int i = 0; i < spaces; i++)
                    System.err.print(" ");
                System.err.println("}");
            ...
            case H5O_TYPE_NAMED_DATATYPE:
                System.err.println("Datatype: " + name);
                break;
            default:
