...
public class DbfMetadataUtils {
	...
	public static void readFields(DbfMetadata metadata, InputStream inputStream) throws IOException {
		...
		boolean L7 = (metadata.getType() == DbfFileTypeEnum.dBASE_LEVEL7);

		List<DbfField> fields = new ArrayList<>();
		byte[] fieldBytes = new byte[(L7) ? JdbfUtils.FIELD_RECORD_LENGTH_L7 : JdbfUtils.FIELD_RECORD_LENGTH];
		
		int headerLength = 0;
		int fieldLength = 0;
		while (true) {
			int n = inputStream.read(fieldBytes);
			if (n < fieldBytes.length) {
				break; // end of data
			}
			if (fieldBytes[0] == JdbfUtils.HEADER_TERMINATOR) {
				break;
			}
			DbfField field = (L7) ? createDbfFieldL7(fieldBytes) : createDbfField(fieldBytes);
			fields.add(field);
	...
	public static DbfField createDbfFieldL7(byte[] fieldBytes) {
		DbfField field = new DbfField();
		// 1. Set name
		{
			int i = 0;
			for (i = 0; i < 31 && fieldBytes[i] > 0; i++)
				; // find first null..
			field.setName(new String(fieldBytes, 0, i));
		}
		// 2. Set type
		field.setType(DbfFieldTypeEnum.fromChar((char) fieldBytes[32]));
		// 3. Set length
		{
			int length = fieldBytes[33];
			if (length < 0) {
				length = 256 + length;
			}
		...
		field.setNumberOfDecimalPlaces(fieldBytes[34]);
