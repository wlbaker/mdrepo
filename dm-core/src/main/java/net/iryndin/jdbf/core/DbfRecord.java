...
public class DbfRecord {
    ...
    */
	...
	public Date getL7Timestamp(String name) {
        DbfField f = getField(name);
        int actualLength = f.getLength();        // should be 8!
        
        byte[] fieldBytes = new byte[actualLength];
        System.arraycopy(bytes, f.getOffset(), fieldBytes, 0, f.getLength());
        
        ByteBuffer bb = ByteBuffer.wrap(fieldBytes);
        // bb.order(ByteOrder.LITTLE_ENDIAN);
        long l1 = bb.getLong();  // number of days since 01/01/4713 BC
        long l2 = bb.getLong(); // hours * 3600000L + minutes * 60000L + Seconds * 1000L
        
        int hr = (int) (l2 / 3600000);
        int min = (int) ((l2 / 60000) % 60);
        int sec = (int) ((l2/1000) % 60);
        
        Calendar cal = Calendar.getInstance();
        cal.set(-4713, 0, 1,hr, min, sec );
        cal.roll( Calendar.DAY_OF_YEAR, (int)l1);
        // FIXME: untested
        System.err.println("CHECK THIS: DATE=" + cal.getTime() );
		return cal.getTime();
	}
	/*
    bb.rewind();
    ...
    // WLB: TESTING A THEORY
    if( fieldBytes[0] == -64 ) {
    	// FIXME: these flags must mean something!
    	l = l / 256;
    } else if( fieldBytes[0] == -128 ) {
    	l = l / 256;
    } else {
    	System.err.println("unexpected timestamp??");
    }
	System.out.print("TSTAMP FIELD: ");
    for( int i = 0; i < 8; i++) {
    	int v = fieldBytes[i];
    	if ( v < 0 ) {
    		v += 256;
