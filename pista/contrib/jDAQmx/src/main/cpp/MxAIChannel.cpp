...
#include "ni_DAQmx_MxAIChannel.h"

#define DAQmxErrChk(functionCall) if( DAQmxFailed(error=(functionCall)) ) goto Error; else

using namespace std;

extern "C" JNIEXPORT jlong JNICALL Java_ni_DAQmx_MxAIChannel_createVoltageChannel(
...
extern "C" JNIEXPORT jdouble JNICALL Java_ni_DAQmx_MxAIChannel_getAccelerationDecibelReference(JNIEnv *env, jobject jobj) {

	jstring channelName = getChannelName( env, jobj );
	const char *nativeChannelName = env->GetStringUTFChars( channelName, 0);

	float64 val;
	DAQmxGetAIAcceldBRef( getTaskHandle( env, jobj ), nativeChannelName, &val );
...
extern "C" JNIEXPORT void JNICALL Java_ni_DAQmx_MxAIChannel_setAccelerationDecibelReference(JNIEnv *env, jobject jobj, jdouble dval ) {
	...
	float64 val = dval;
	DAQmxSetAIAcceldBRef( getTaskHandle( env, jobj ), nativeChannelName, val);
