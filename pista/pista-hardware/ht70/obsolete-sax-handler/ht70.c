...
static void ht70_parse_wave( Ht70__Alarms *a, char *name, char *value ) {
...
static void chars(char * chars) {

    char *top = stack[tabs-1];
    char *prev = stack[tabs-2];

    if( strcmp(top,"s") == 0 ) {
        ht70_parse_setting( &me->s, element_name, chars);
    } else if( strcmp(prev,"mg") == 0 ) {
        ht70_parse_wave( NULL, element_name, chars);
    } else if( strcmp(prev,"measurements") == 0 ) {
        ht70_parse_measurement( &me->m, element_name, chars);
    } else if( strcmp(prev,"alarms") == 0 ) {
        ht70_parse_alarms( &me->a, element_name, chars);
    } else if( strcmp(prev,"cpc") == 0 ) {
        // ignore
    } else if( strcmp(prev,"device") == 0 ) {
        // ignore
    } else {
        printf("unknown element: %s/%s name=%s", prev, top, element_name );
    }
}

static void start_elem(char * name, attr_array *attrs) {
    bool show_it = false;

    stack[tabs] = strdup(name);
	tabs++;
    if( strcmp(name, "s") == 0 ) {
	...
	for (int i = 0; i < attrs->size; i++) {
	    if( show_it) printf(" %s=\"%s\"\n", attrs->attrs[i].name, attrs->attrs[i].value);
        if( strcmp( "name", attrs->attrs[i].name) == 0 ) {
            strncpy( element_name, attrs->attrs[i].value,32 );
        }
	}
	if( show_it) printf(">\n");
}

static void end_elem(char *name) {
	tabs--;

    free(stack[tabs]);
    stack[tabs] = NULL;
    strcpy( element_name, "<none>" );

    if( strcmp( name, "settings" ) == 0 ) {
        printf("fire settings\n");
    } else if( strcmp( name, "mg" ) == 0 ) {
        printf("[got mg=?]");
    } else if( strcmp( name, "measurements" ) == 0 ) {
        printf("fire measurements\n");
    } else if( strcmp( name, "alarms" ) == 0 ) {
        printf("fire alarms\n");
    } else if( strcmp( name, "device" ) == 0 ) {
        // ignore
...
static void ht70_install(struct HT70_context *context) {
    me = context;
}
